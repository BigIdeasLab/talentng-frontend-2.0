...
*Zero-risk fixes that can be shipped immediately. No refactoring, no moving files.*

### Group 1A: Typos & Identity Fixes

| # | Task | File(s) | Est. |
|---|------|---------|------|
| - [ ] 1 | Fix `"Private Policy"` → `"Privacy Policy"` | `app/page.tsx` | 1 min |
| - [ ] 2 | Fix `"Terms od Service"` → `"Terms of Service"` | `app/page.tsx` | 1 min |
| - [ ] 3 | Change package name `"fusion-starter"` → `"talentng-frontend"` | `package.json` | 2 min |
| - [ ] 4 | Fix `not-found.tsx` placeholder text (`"NOT IMPLEMENTED YET"`) — replace with proper 404 copy | `app/not-found.tsx` | 5 min |

### Group 1B: Dependency Placement

| # | Task | File(s) | Est. |
|---|------|---------|------|
| - [ ] 5 | Move `react`, `react-dom`, `react-hook-form`, `framer-motion`, `sonner`, `date-fns`, `lucide-react`, `recharts`, `@radix-ui/*`, `class-variance-authority`, `clsx`, `tailwind-merge`, `next-themes`, `input-otp`, `@tanstack/react-query` from `devDependencies` → `dependencies` | `package.json` | 15 min |

### Group 1C: Dead Code & Stale Files

| # | Task | File(s) | Est. |
|---|------|---------|------|
| - [ ] 6 | Delete `temp_talent_opp.txt` | project root | 1 min |
| - [ ] 7 | Delete `API_TEST_RESULTS.json` | project root | 1 min |
| - [ ] 8 | Delete `-p/` directory | project root | 1 min |
| - [ ] 9 | Investigate `admin-frontend` file — delete if empty/stale | project root | 2 min |
| - [ ] 10 | Remove commented-out SSE/WebSocket code in `layout-client.tsx` (lines 69–105), or create a GitLab issue and add a `// TODO(#issue)` reference | `app/(business)/layout-client.tsx` | 5 min |

### Group 1D: Duplicate JWT Decode

| # | Task | File(s) | Est. |
|---|------|---------|------|
| - [ ] 11 | Delete `decodeJwt()` from `lib/utils.ts` | `lib/utils.ts` | 2 min |
| - [ ] 12 | Find all imports of `decodeJwt` and replace with `decodeToken` + `isTokenExpired` from `lib/auth.ts` | grep across codebase | 10 min |
...
*Eliminate duplicated auth patterns. No folder restructuring yet.*
...
| # | Task | File(s) | Est. |
|---|------|---------|------|
| - [ ] 13 | Create `forceLogout()` function in `lib/auth.ts` that clears localStorage + cookies + redirects to `/login` | `lib/auth.ts` | 10 min |
| - [ ] 14 | Replace all 3 inline occurrences in `lib/api/index.ts` with `forceLogout()` | `lib/api/index.ts` | 10 min |
...
| # | Task | File(s) | Est. |
|---|------|---------|------|
| - [ ] 15 | Replace direct `document.cookie` reads/writes in `lib/api/index.ts` with `getCookie`/`setCookie`/`deleteCookie` from `lib/utils.ts` | `lib/api/index.ts` | 15 min |
| - [ ] 16 | Replace direct `document.cookie` in `components/GlobalErrorHandler.tsx` | `components/GlobalErrorHandler.tsx` | 5 min |
| - [ ] 17 | Replace direct `document.cookie` in `app/(business)/profile-provider.tsx` | `app/(business)/profile-provider.tsx` | 5 min |
...
| # | Task | File(s) | Est. |
|---|------|---------|------|
| - [ ] 18 | Remove `hasToken` constant; set `enabled: true` directly (or remove it) | `hooks/useAuth.ts` | 5 min |
| - [ ] 19 | Remove the useEffect that re-fetches based on `hasToken` — it fires every render because `refetchUser` reference changes | `hooks/useAuth.ts` | 10 min |
| - [ ] 20 | Verify auth flow still works (login → dashboard → refresh) | Manual testing | 10 min |
...
*Clean the utility dumping ground and move loose files into proper subfolders.*
...
| # | Task | File(s) | Est. |
|---|------|---------|------|
| - [ ] 21 | Move `getCookie`, `setCookie`, `deleteCookie` → `lib/auth/cookies.ts` (new file) | `lib/utils.ts` → `lib/auth/cookies.ts` | 15 min |
| - [ ] 22 | Move `formatDateForDisplay`, `formatDuration` → `lib/utils/date.ts` (new file) | `lib/utils.ts` → `lib/utils/date.ts` | 10 min |
| - [ ] 23 | Update all imports across the codebase to point to new locations | grep + update | 20 min |
| - [ ] 24 | Confirm `lib/utils.ts` now only exports `cn()` | `lib/utils.ts` | 2 min |

### Group 3B: Organise Loose `lib/` Files

| # | Task | File(s) | Est. |
|---|------|---------|------|
| - [ ] 25 | Move `lib/auth.ts` → `lib/auth/tokens.ts` | rename + update imports | 10 min |
| - [ ] 26 | Move `lib/auth.test.ts` → `lib/auth/tokens.test.ts` | rename + update | 2 min |
| - [ ] 27 | Move `lib/token-refresh.ts` → `lib/auth/token-refresh.ts` | rename + update imports | 10 min |
| - [ ] 28 | Move `lib/device.ts` → `lib/auth/device.ts` | rename + update imports | 5 min |
| - [ ] 29 | Move `lib/profileMapper.ts` → `lib/mappers/profile.ts` | rename + update imports | 10 min |
| - [ ] 30 | Rename `lib/utils.spec.ts` → `lib/utils.test.ts` (standardise on `.test`) | rename | 1 min |

### Group 3C: Extract `buildQueryString()` Utility

| # | Task | File(s) | Est. |
|---|------|---------|------|
| - [ ] 31 | Create `buildQueryString(params)` in `lib/utils/query.ts` | new file | 10 min |
| - [ ] 32 | Replace 3 repeated `URLSearchParams` blocks in `lib/api/opportunities/index.ts` | `lib/api/opportunities/index.ts` | 10 min |
...
*Single source of truth for every domain type. This is the highest-complexity phase.*

### Group 4A: Audit Current State

| # | Task | File(s) | Est. |
|---|------|---------|------|
| - [ ] 33 | Map every `interface Opportunity` / `type Opportunity` across the codebase — note which fields each has | investigation | 20 min |
| - [ ] 34 | Map every `interface Application` / `type Application` | investigation | 15 min |
| - [ ] 35 | Map every `interface User` / `type User` | investigation | 10 min |
...
| # | Task | File(s) | Est. |
|---|------|---------|------|
| - [ ] 36 | Merge `lib/types/opportunity.ts` + `lib/types/opportunities.ts` into a single `lib/types/opportunity.ts` (UI types only) | 2 files → 1 | 20 min |
| - [ ] 37 | Delete `lib/api/types/opportunity.ts` — merge its definitions into `lib/api/opportunities/types.ts` | delete + merge | 15 min |
| - [ ] 38 | Update `components/talent/opportunities/types.ts` to re-export from canonical location (or delete if redundant) | 1 file | 10 min |
| - [ ] 39 | Update all imports codebase-wide | grep + update | 20 min |

### Group 4C: Consolidate Application & User Types

| # | Task | File(s) | Est. |
|---|------|---------|------|
| - [ ] 40 | Delete `lib/api/types/application.ts` — merge into `lib/api/applications/types.ts` | delete + merge | 15 min |
| - [ ] 41 | Reconcile `lib/types/application.ts` with `lib/api/applications/types.ts` — keep one canonical shape | 2 files | 15 min |
| - [ ] 42 | Reconcile `lib/types/auth.ts` User type with `lib/api/applications/types.ts` User type | 2 files | 10 min |
| - [ ] 43 | Update all imports codebase-wide | grep + update | 15 min |

### Group 4D: Delete `lib/api/types/` Folder

| # | Task | File(s) | Est. |
|---|------|---------|------|
| - [ ] 44 | Ensure everything from `lib/api/types/` is merged into domain folders | verify | 5 min |
| - [ ] 45 | Delete `lib/api/types/` directory | delete | 1 min |
| - [ ] 46 | Run TypeScript compiler — fix any broken imports | `npx tsc --noEmit` | 15 min |
...
| # | Task | File(s) | Est. |
|---|------|---------|------|
| - [ ] 47 | Move all functions from `lib/api/auth-service.ts` into `lib/api/auth/index.ts` | 2 files → 1 | 20 min |
| - [ ] 48 | Delete `lib/api/auth-service.ts` | delete | 1 min |
| - [ ] 49 | Update all imports | grep + update | 15 min |
...
| # | Task | File(s) | Est. |
|---|------|---------|------|
| - [ ] 50 | Create `lib/api/errors.ts` with typed `ApiError` class (fields: `status`, `data`, `isRateLimit`, `isRoleMismatch`, `retryAfter`, `actualRole`, `requiredRole`) | new file | 15 min |
| - [ ] 51 | Update `lib/api/index.ts` to throw `ApiError` instead of setting `any` properties on `Error` | `lib/api/index.ts` | 20 min |
| - [ ] 52 | Update `GlobalErrorHandler.tsx` and other error consumers to use `ApiError` type guard | 3-5 files | 15 min |

### Group 5C: Standardise API Function Style

| # | Task | File(s) | Est. |
|---|------|---------|------|
| - [ ] 53 | Convert arrow function API exports in `lib/api/opportunities/index.ts` to function declarations (to match `lib/api/talent/index.ts` style) | `lib/api/opportunities/index.ts` | 15 min |
| - [ ] 54 | Scan other API files (`lib/api/mentor/`, `lib/api/recruiter/`, `lib/api/applications/`) for arrow functions and convert | 3-4 files | 15 min |

### Group 5D: Fix Base URL Inconsistency

| # | Task | File(s) | Est. |
|---|------|---------|------|
| - [ ] 55 | Align fallback URLs in `lib/api/index.ts` and `lib/api/server-client.ts` — both should use the same default (e.g., `http://localhost:3001`) | 2 files | 5 min |
...
*Break the 1112-line monolith into testable, maintainable components.*

### Group 6A: Extract Static Data

| # | Task | File(s) | Est. |
|---|------|---------|------|
| - [ ] 56 | Create `lib/data/landing.ts` with all data arrays: `navLinks`, `painPoints`, `roles`, `howItWorksSteps`, `categories`, `talents`, `faqs`, `footerLinks` | new file | 30 min |

### Group 6B: Extract SVG Icons

| # | Task | File(s) | Est. |
|---|------|---------|------|
| - [ ] 57 | Create `components/landing/icons.tsx` — move all 6 inline SVG icon components | new file | 15 min |

### Group 6C: Extract Sections

| # | Task | File(s) | Est. |
|---|------|---------|------|
| - [ ] 58 | Create `components/landing/HeroSection.tsx` | new file | 15 min |
| - [ ] 59 | Create `components/landing/ProblemSection.tsx` (pain points) | new file | 15 min |
| - [ ] 60 | Create `components/landing/RolesSection.tsx` | new file | 15 min |
| - [ ] 61 | Create `components/landing/HowItWorksSection.tsx` | new file | 15 min |
| - [ ] 62 | Create `components/landing/DiscoverSection.tsx` | new file | 15 min |
| - [ ] 63 | Create `components/landing/FAQSection.tsx` | new file | 15 min |
| - [ ] 64 | Create `components/landing/CTASection.tsx` | new file | 10 min |
| - [ ] 65 | Create `components/landing/LandingNavbar.tsx` | new file | 10 min |
| - [ ] 66 | Create `components/landing/LandingFooter.tsx` | new file | 10 min |

### Group 6D: Simplify `app/page.tsx`

| # | Task | File(s) | Est. |
|---|------|---------|------|
| - [ ] 67 | Rewrite `app/page.tsx` to import and compose sections (~30–50 lines) | `app/page.tsx` | 15 min |
| - [ ] 68 | Visually verify landing page renders identically | Manual testing | 15 min |
...
| # | Task | File(s) | Est. |
|---|------|---------|------|
| - [ ] 69 | Rename kebab-case `.tsx` component files to PascalCase (e.g., `application-modal.tsx` → `ApplicationModal.tsx`, `opportunity-card.tsx` → `OpportunityCard.tsx`, `search-input.tsx` → `SearchInput.tsx`) | ~15 files | 30 min |
| - [ ] 70 | Rename camelCase `.ts` utility files to kebab-case (e.g., `profileMapper.ts` → already moved in Phase 3) | verify | 5 min |
| - [ ] 71 | Rename `DiscoverTalent/` folder → `discover-talent/` | 1 folder | 10 min |
| - [ ] 72 | Update all imports affected by renames | grep + update | 20 min |
...
*These are significant refactors. Schedule as separate sprints.*
...
*Ready to start? Run Phase 1 first — it's all quick wins with zero risk.*
