...
interface BestWorkUploadProps {
  isEditing: boolean;
  profile: TalentProfile | undefined;
}

export function BestWorkUpload({ isEditing, profile }: BestWorkUploadProps) {
  const { user } = useAuth();
  ...
  const handleSubmit = () => {
    if (selectedFiles.length > 0) {
      galleryUploadMutation.mutate(selectedFiles);
    }
  };

  return (
    <div className="space-y-6 mt-8">
      <h2 className="text-2xl font-medium text-black font-geist">Best Work</h2>
      <p className="text-base font-medium text-gray-500 font-geist">
        Showcase your best work by uploading images.
      </p>
        ...
        {isEditing && (
          <div className="space-y-4">
            <div
              ...
              className="w-full flex flex-col items-center justify-center gap-6 py-9 px-0 border border-dashed border-black rounded-3xl bg-gray-50 hover:bg-gray-100 transition-colors cursor-pointer"
            ...
            <button
              ...
              disabled={
                selectedFiles.length === 0 || galleryUploadMutation.isPending
              }
              className="w-full px-4 py-2 border border-gray-300 rounded-lg bg-white text-sm font-medium hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed"
            >
