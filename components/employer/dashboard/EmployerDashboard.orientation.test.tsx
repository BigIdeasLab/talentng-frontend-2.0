...
vi.mock('./WeeklyOverviewChart', () => ({
  WeeklyOverviewChart: ({ data }: any) => <div data-testid="weekly-chart">Weekly Chart</div>,
}));

vi.mock('./HiringPipeline', () => ({
  HiringPipeline: ({ data }: any) => <div data-testid="hiring-pipeline">Hiring Pipeline</div>,
}));

vi.mock('./TopOpportunities', () => ({
  TopOpportunities: ({ data }: any) => <div data-testid="top-opportunities">Top Opportunities</div>,
}));

vi.mock('./RecentActivity', () => ({
  RecentActivity: ({ data }: any) => <div data-testid="recent-activity">Recent Activity</div>,
}));
...
describe('EmployerDashboard - Orientation Tests', () => {
  ...
  it('should render stats cards in single column on portrait mobile', async () => {
    ...
    expect(screen.getByTestId('stats-card-total-applicants')).toBeInTheDocument();
    expect(screen.getByTestId('stats-card-active-opportunities')).toBeInTheDocument();
    expect(screen.getByTestId('stats-card-hired-this-month')).toBeInTheDocument();
    expect(screen.getByTestId('stats-card-pending-reviews')).toBeInTheDocument();
  });
