...
vi.mock("@/hooks/useRecruiterOpportunities", () => ({
  useRecruiterOpportunitiesQuery: vi.fn(() => ({
    data: { data: [], pagination: { hasNextPage: false, hasPreviousPage: false } },
    isLoading: false,
    isPending: false,
    refetch: vi.fn(),
  })),
...
describe("Property-Based Tests: Recruiter Opportunities Improvements", () => {
    ...
    await fc.assert(
      fc.asyncProperty(
        fc.record({
          ...
          types: fc.array(fc.constantFrom("Full-time", "Part-time", "Contract"), { maxLength: 2 }),
        }),
        async (filters) => {
          const onApply = vi.fn();
          const onClose = vi.fn();
      ...
      fc.asyncProperty(
        ...
        async ({ isInitialLoad, hasData }) => {
          ...
          if (hasData) {
            ...
            const shouldShowSkeletonAfterLoad = isInitialLoadRef.current && !hasData;
  ...
  it("Property 10: Empty State with Active Filters", async () => {
    await fc.assert(
      fc.asyncProperty(
        fc.record({
          skills: fc.array(fc.string({ minLength: 1 }), { minLength: 1, maxLength: 3 }),
          types: fc.array(fc.string({ minLength: 1 }), { minLength: 1, maxLength: 2 }),
        }),
        async (filters) => {
          // Property: When filters are active and no results, show specific message
          const hasActiveFilters = filters.skills.length > 0 || filters.types.length > 0;
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async (searchQuery) => {
          ...
          expect(expectedDescription).toBe("Try adjusting your search query or filters");
        }
      ),
      { numRuns: 100 }
    );
