...
export function OpportunityDetails({
  ...
  const [isApplied, setIsApplied] = useState(
    initialAppliedAs?.includes(currentProfileType) ?? false,
  );
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_450px] gap-5 lg:gap-2">
          ...
          <div className="flex flex-col gap-9 overflow-y-auto pr-2">
            ...
            <div className="flex flex-col gap-4">
              <div className="flex flex-col gap-4">
                <Link
                  href={`/recruiter/${opportunity.postedById || opportunity.postedBy?.id}`}
                  className="w-[75px] h-[75px] rounded-full overflow-hidden hover:opacity-80 transition-opacity cursor-pointer"
                >
                  <img
                    src={getCompanyLogo()}
                    alt="Company Logo"
                ...
                <div className="flex flex-col gap-3">
                  ...
                  <div className="flex items-center gap-1.5">
                    <Link
                      href={`/recruiter/${opportunity.postedById || opportunity.postedBy?.id}`}
                      className="font-inter-tight text-[15px] font-normal text-black/30 hover:text-black/50 transition-colors cursor-pointer"
                    >
                      {opportunity.company}
                    </Link>
          ...
          <div className="flex flex-col gap-2 sticky top-0 h-fit">
            ...
            <div className="border border-[#E1E4EA] rounded-[16px] p-4 flex flex-col gap-5">
              <Link
                href={`/recruiter/${opportunity.postedBy?.id}`}
