...
test.describe("Component Visibility Tests - Visual Regression", () => {
  ...
  test.describe("Desktop Sidebar Visibility", () => {
    ...
    test("Desktop sidebar - collapsed on tablet", async ({ page }) => {
      ...
      const sidebarLabels = page
        .locator("aside span, .sidebar span")
        .filter({ hasText: /Dashboard|Opportunities|Applicants/ });
    ...
    test("Sidebar logo visibility - hidden on tablet, shown on desktop", async ({
      page,
    }) => {
      ...
      const logo = page.locator(
        'aside img[alt*="Logo"], .sidebar img[alt*="Logo"]',
      );
  ...
  test.describe("Mobile-Specific Components", () => {
    ...
    test("Mobile tabs - shown only on mobile", async ({ page }) => {
      ...
      const mobileTabs = page.locator(
        '[data-testid="mobile-tabs"], .mobile-tabs, .profile-tabs.md\\:hidden',
      );
  ...
  test.describe("Table Column Visibility", () => {
    ...
    test("Table columns - essential columns only on tablet", async ({
      page,
    }) => {
      await testPageResponsive(
        page,
      ...
      const tableContainer = page.locator(
        '[data-testid="applicants-table"], .applicants-table, .table-container',
      );
    ...
    test("Opportunities table - responsive transformation", async ({
      page,
    }) => {
      await testPageResponsive(
        page,
        ...
        VIEWPORTS.mobile,
        {
          ...
          userType: "employer",
          waitForSelector:
            '[data-testid="opportunities-table"], .opportunities-table, .opportunity-card',
          fullPage: true,
        },
      );

      // Verify opportunity cards are visible on mobile
      const opportunityCards = page.locator(
  ...
  test.describe("Stat Descriptions Visibility", () => {
    test("Stat descriptions - hidden on mobile", async ({ page }) => {
      ...
      const statDescriptions = page.locator(
        '.stat-description, [data-testid="stat-description"]',
      );
      ...
      const statValues = page.locator(
        '.stat-value, [data-testid="stat-value"]',
      );
    ...
    test("Stat descriptions - shown on tablet and desktop", async ({
      page,
    }) => {
      await testPageResponsive(
        page,
  ...
  test.describe("Decorative Elements Visibility", () => {
    test("Decorative elements - hidden on mobile", async ({ page }) => {
      ...
      const decorativeElements = page.locator(
        '.decorative, [data-testid="decorative"], .illustration, .background-pattern',
      );
  ...
  test.describe("Visibility Utility Components", () => {
    test("HideOnMobile component - hidden on mobile, shown on desktop", async ({
      page,
    }) => {
    ...
    test("ShowOnMobile component - shown on mobile, hidden on desktop", async ({
      page,
    }) => {
  ...
  test.describe("Profile Switcher Visibility", () => {
    test("Profile switcher - hidden on tablet sidebar, shown on desktop", async ({
      page,
    }) => {
      await testPageResponsive(
        page,
      ...
      const profileSwitcher = page.locator(
        'aside [data-testid="profile-switcher"], .sidebar .profile-switcher',
      );
    ...
    test("Profile switcher - shown in mobile drawer", async ({ page }) => {
      ...
      const mobileProfileSwitcher = page.locator(
        '[role="dialog"] [data-testid="profile-switcher"], .mobile-drawer .profile-switcher',
      );
  ...
  test.describe("Navigation Badge Visibility", () => {
    test("Navigation badges - responsive positioning", async ({ page }) => {
      ...
      const notificationBadges = page.locator(
        'aside .badge, .sidebar .badge, aside [class*="bg-"][class*="rounded-full"]',
      );
      if ((await notificationBadges.count()) > 0) {
        // Verify badges have responsive positioning classes
        await expect(notificationBadges.first()).toHaveClass(
          /md:absolute|lg:static/,
        );
      }
    });
  });
  ...
  test.describe("Cross-Viewport Consistency", () => {
    test("Component visibility consistency across user types", async ({
      page,
    }) => {
      ...
      for (const userType of userTypes) {
        await testPageResponsive(
          ...
          VIEWPORTS.mobile,
          {
            ...
            userType,
            waitForSelector:
              '[data-testid="dashboard-stats"], .dashboard-stats',
            fullPage: true,
          },
        );
