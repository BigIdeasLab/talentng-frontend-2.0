...
import { VIEWPORTS, waitForPageStable, mockAuthentication } from './helpers/test-utils';
...
test.describe('Touch Behavior Interactions', () => {
  ...
  test.describe('Mobile Drawer Touch Behaviors', () => {
    ...
    test('should open mobile drawer with hamburger button tap', async ({ page }) => {
      ...
      const hamburgerButton = page.locator('[data-testid="hamburger-menu"]').first();
    ...
    test('should support swipe-to-close gesture on mobile drawer', async ({ page }) => {
      await page.setViewportSize(VIEWPORTS.mobile);
      await mockAuthentication(page, 'employer');
      await page.goto('/business/dashboard');
      await waitForPageStable(page);
    ...
    test('should have touch-friendly navigation items in drawer', async ({ page }) => {
      await page.setViewportSize(VIEWPORTS.mobile);
      await mockAuthentication(page, 'employer');
      await page.goto('/business/dashboard');
      await waitForPageStable(page);
  ...
  test.describe('Swipe Gesture Behaviors', () => {
    ...
    test('should support swipe-to-dismiss on notification items', async ({ page }) => {
      await page.setViewportSize(VIEWPORTS.mobile);
      await mockAuthentication(page, 'employer');
      await page.goto('/business/dashboard');
      await waitForPageStable(page);
      ...
      const notificationButton = page.locator('[data-testid="notifications-button"]').first();
      if (await notificationButton.isVisible()) {
        await notificationButton.tap();
        await page.waitForTimeout(300);
        
        // Find swipeable notification item
        const notificationItem = page.locator('[data-testid="swipeable-notification"]').first();
    ...
    test('should support swipe-to-dismiss on modal sheets', async ({ page }) => {
      await page.setViewportSize(VIEWPORTS.mobile);
      await mockAuthentication(page, 'employer');
  ...
  test.describe('Touch-Friendly Tap Targets', () => {
    ...
    test('should have minimum 44x44px tap targets for all buttons', async ({ page }) => {
      await page.setViewportSize(VIEWPORTS.mobile);
      await mockAuthentication(page, 'employer');
    ...
    test('should have adequate spacing between interactive elements', async ({ page }) => {
      ...
      const actionGroups = page.locator('[data-testid*="actions"], .action-group, .button-group');
  ...
  test.describe('Modal Touch Interactions', () => {
    ...
    test('should have touch-friendly close button in modals', async ({ page }) => {
      await page.setViewportSize(VIEWPORTS.mobile);
      await mockAuthentication(page, 'employer');
      await page.goto('/business/applicants');
      await waitForPageStable(page);
      ...
      if (await modalTrigger.isVisible()) {
        ...
        if (await modal.isVisible()) {
          // Find close button
          const closeButton = modal.locator('[data-testid="close"], button[aria-label*="close"], .close-button').first();
    ...
    test('should stack form fields vertically in mobile modals', async ({ page }) => {
      await page.setViewportSize(VIEWPORTS.mobile);
      await mockAuthentication(page, 'employer');
      await page.goto('/business/applicants');
      await waitForPageStable(page);
  ...
  test.describe('Touch Feedback and Active States', () => {
    ...
    test('should provide touch feedback on interactive elements', async ({ page }) => {
      await page.setViewportSize(VIEWPORTS.mobile);
      await mockAuthentication(page, 'employer');
      await page.goto('/business/opportunities');
      await waitForPageStable(page);
  ...
  test.describe('Accessibility and Screen Reader Support', () => {
    ...
    test('should have proper ARIA labels on touch elements', async ({ page }) => {
      await page.setViewportSize(VIEWPORTS.mobile);
      await mockAuthentication(page, 'employer');
      await page.goto('/business/dashboard');
      await waitForPageStable(page);
    ...
    test('should support keyboard navigation on tablet viewports', async ({ page }) => {
