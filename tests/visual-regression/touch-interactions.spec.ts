...
import { testPageResponsive, VIEWPORTS, waitForPageStable, mockAuthentication } from './helpers/test-utils';
...
test.describe('Touch Interactions', () => {
  ...
  test.describe('Swipe Gestures', () => {
    ...
    test('Swipeable notification item - dismiss gesture', async ({ page }) => {
      ...
      await page.addInitScript(() => {
        ...
        localStorage.setItem('notifications', JSON.stringify([
          { id: '1', title: 'Test Notification', message: 'Test message', type: 'info' }
        ]));
      });
      ...
      const notificationButton = page.locator('[data-testid="notifications-button"], .notifications-trigger').first();
      if (await notificationButton.isVisible()) {
        await notificationButton.click();
        await page.waitForTimeout(300);
        
        // Find swipeable notification item
        const notificationItem = page.locator('[data-testid="swipeable-notification"], .swipeable-notification-item').first();
    ...
    test('Swipeable modal - dismiss gesture', async ({ page }) => {
      ...
      const filterButton = page.locator('[data-testid="filter-button"], .filter-trigger, button:has-text("Filter")').first();
      if (await filterButton.isVisible()) {
        await filterButton.click();
        await page.waitForTimeout(300);
        
        const modal = page.locator('[data-testid="swipeable-modal"], .swipeable-modal, [role="dialog"]').first();
  ...
  test.describe('Modal Touch Interactions', () => {
    ...
    test('Modal full-screen behavior on mobile', async ({ page }) => {
      ...
      // Open a modal
      const modalTrigger = page.locator('[data-testid*="modal"], button:has-text("Filter"), button:has-text("Schedule")').first();
    ...
    test('Modal close button touch target', async ({ page }) => {
      ...
      // Open a modal
      const modalTrigger = page.locator('[data-testid*="modal"], button:has-text("Filter")').first();
      if (await modalTrigger.isVisible()) {
        await modalTrigger.click();
        await page.waitForTimeout(300);
    ...
    test('Modal scrolling behavior on mobile', async ({ page }) => {
      ...
      if (await longModal.isVisible()) {
        ...
        const scrollableArea = longModal.locator('.modal-content, .modal-body, [data-testid="modal-content"]').first();
  ...
  test.describe('Dropdown and Menu Touch Interactions', () => {
    ...
    test('Dropdown menu touch-friendly targets', async ({ page }) => {
      ...
      const dropdownTrigger = page.locator('[data-testid*="dropdown"], .dropdown-trigger, button:has-text("Actions")').first();
