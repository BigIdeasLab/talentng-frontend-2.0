...
test.describe('Core Orientation Change Tests', () => {
  ...
  test('should handle basic orientation change from portrait to landscape', async ({ page }) => {
  ...
  test('should preserve form data during orientation change', async ({ page }) => {
    await page.goto('/login');
    await waitForPageStable(page);
  ...
  test('should maintain touch-friendly targets in both orientations', async ({ page }) => {
    await page.goto('/login');
    await waitForPageStable(page);
    ...
    const testTouchTargets = async (orientation: string) => {
      ...
      if (await submitButton.isVisible()) {
        ...
        if (boundingBox) {
          expect(boundingBox.height, `Submit button height in ${orientation}`).toBeGreaterThanOrEqual(40);
          expect(boundingBox.width, `Submit button width in ${orientation}`).toBeGreaterThanOrEqual(40);
        }
      }
      ...
      if (await emailInput.isVisible()) {
        ...
        if (boundingBox) {
          expect(boundingBox.height, `Email input height in ${orientation}`).toBeGreaterThanOrEqual(40);
        }
      }
    };
  ...
  test('should handle rapid orientation changes gracefully', async ({ page }) => {
    await page.goto('/login');
    await waitForPageStable(page);
