...
test.describe('Profile Pages - Visual Regression', () => {
    ...
    test(`Employer profile - ${viewport.name}`, async ({ page }) => {
      await testPageResponsive(
        ...
        viewport,
        {
          mockAuth: true,
          userType: 'employer',
          waitForSelector: '[data-testid="profile-header"], .profile-header, .profile-content',
          fullPage: true,
        }
      );
    });
  ...
  Object.values(VIEWPORTS).forEach((viewport) => {
    test(`Talent profile view (public) - ${viewport.name}`, async ({ page }) => {
      await testPageResponsive(
        page,
        ...
        viewport,
        {
          mockAuth: true,
          userType: 'employer',
          waitForSelector: '[data-testid="talent-profile-view"], .talent-profile-view, .profile-header',
          fullPage: true,
        }
      );
    });
  ...
  test('Profile navigation tabs - mobile', async ({ page }) => {
    await testPageResponsive(
      ...
      VIEWPORTS.mobile,
      {
        mockAuth: true,
        userType: 'talent',
        waitForSelector: '[data-testid="profile-tabs"], .profile-tabs, .profile-navigation',
        fullPage: true,
      }
    );
  });
  ...
  test('Profile stats grid - mobile vs desktop', async ({ page }) => {
    ...
    await testPageResponsive(
      ...
      VIEWPORTS.mobile,
      {
        mockAuth: true,
        userType: 'talent',
        waitForSelector: '[data-testid="profile-stats"], .profile-stats, .stats-grid',
  ...
  test('Profile with portfolio - tablet', async ({ page }) => {
    await testPageResponsive(
      ...
      VIEWPORTS.tablet,
      {
        mockAuth: true,
        userType: 'talent',
        waitForSelector: '[data-testid="portfolio-section"], .portfolio-section, .works-grid',
        interactions: async (page) => {
          // Navigate to portfolio tab if it exists
          const portfolioTab = page.locator('[data-testid="portfolio-tab"], button:has-text("Portfolio")').first();
  ...
  test('Profile image upload - mobile', async ({ page }) => {
    await testPageResponsive(
      ...
      VIEWPORTS.mobile,
      {
        mockAuth: true,
        userType: 'talent',
        waitForSelector: '[data-testid="profile-image-upload"], .profile-image-upload, .avatar-upload',
        interactions: async (page) => {
          // Click on profile image upload area
          const uploadArea = page.locator('[data-testid="profile-image-upload"], .profile-image-upload, .avatar-upload').first();
    ...
    test(`Discover talent page - ${viewport.name}`, async ({ page }) => {
      await testPageResponsive(
        ...
        viewport,
        {
          mockAuth: true,
          userType: 'employer',
          waitForSelector: '[data-testid="talent-grid"], .talent-grid, .talent-cards',
          fullPage: true,
        }
      );
    });
    ...
    await testPageResponsive(
      ...
      VIEWPORTS.desktop,
      {
        ...
        interactions: async (page) => {
          ...
          const firstCard = page.locator('[data-testid="talent-card"], .talent-card').first();
