...
import { testPageResponsive, VIEWPORTS, waitForPageStable, hideDynamicContent, mockAuthentication } from './helpers/test-utils';

test.describe('Orientation Change Tests', () => {
  
  test.describe('Orientation Change Event Handling', () => {
    test('should handle orientation change from portrait to landscape on mobile', async ({ page }) => {
      ...
      await expect(page).toHaveScreenshot('orientation-landscape-after-change.png');
    ...
    test('should handle rapid orientation changes without breaking', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="dashboard-stats"], .dashboard-stats, .stats-grid')).toBeVisible();
      await expect(page).toHaveScreenshot('orientation-rapid-changes-final.png');
    });

    test('should preserve page functionality during orientation transitions', async ({ page }) => {
      ...
      await expect(page).toHaveScreenshot('orientation-functionality-preserved.png');
    });
  });

  test.describe('State Preservation During Orientation Changes', () => {
    test('should preserve form data during orientation changes', async ({ page }) => {
      await page.goto('/login');
      await waitForPageStable(page);
      ...
      await expect(page).toHaveScreenshot('orientation-form-data-preserved.png');
    });

    test('should preserve scroll position during orientation changes', async ({ page }) => {
      await page.goto('/business/opportunities');
      await mockAuthentication(page, 'employer');
      await page.reload();
      await waitForPageStable(page);
    ...
    test('should preserve modal state during orientation changes', async ({ page }) => {
      ...
      const filterButton = page.locator('button:has-text("Filter"), button[aria-label*="filter"], [data-testid="filter-button"]').first();
      if (await filterButton.isVisible()) {
        ...
        await expect(page).toHaveScreenshot('orientation-modal-state-preserved.png');
      }
    });
  });

  test.describe('Layout Adaptation in Landscape Mode', () => {
    test('should adapt dashboard layout for landscape mobile', async ({ page }) => {
      await page.goto('/business/dashboard');
      await mockAuthentication(page, 'employer');
      await page.reload();
      await waitForPageStable(page);
      ...
      const statsGrid = page.locator('[data-testid="dashboard-stats"], .dashboard-stats, .stats-grid').first();
    ...
    test('should adapt data tables for landscape mobile', async ({ page }) => {
      ...
      await waitForPageStable(page);
      await hideDynamicContent(page);
      await expect(page).toHaveScreenshot('applicants-table-portrait-mobile.png');
      
      // Landscape mobile - should show more tablet-like layout
      await page.setViewportSize({ width: 812, height: 375 });
      await waitForPageStable(page);
      await hideDynamicContent(page);
      await expect(page).toHaveScreenshot('applicants-table-landscape-mobile.png');
      
      // Verify table/card layout adaptation
      const tableContainer = page.locator('table, [data-testid*="table"], .table-container, .responsive-table').first();
    ...
    test('should adapt calendar layout for landscape mobile', async ({ page }) => {
      ...
      const calendarGrid = page.locator('.calendar-grid, [data-testid="calendar-grid"], .calendar-container').first();
  ...
  test.describe('Cross-Page Orientation Testing', () => {
    const testPages = [
      ...
      { path: '/business/dashboard', name: 'dashboard', waitFor: '[data-testid="dashboard-stats"], .dashboard-stats, .stats-grid', auth: true },
      { path: '/business/opportunities', name: 'opportunities', waitFor: 'main, .main-content', auth: true },
      { path: '/business/applicants', name: 'applicants', waitFor: 'main, .main-content', auth: true },
      { path: '/business/profile', name: 'profile', waitFor: 'main, .main-content', auth: true },
    ];

    testPages.forEach(({ path, name, waitFor, auth }) => {
      test(`should handle orientation changes on ${name} page`, async ({ page }) => {
        if (auth) {
          await mockAuthentication(page, 'employer');
        }
        ...
        await expect(page).toHaveScreenshot(`${name}-portrait-before-change.png`);
        ...
        await expect(page).toHaveScreenshot(`${name}-landscape-after-change.png`);
        ...
        await expect(page).toHaveScreenshot(`${name}-portrait-after-landscape.png`);
  ...
  test.describe('Navigation During Orientation Changes', () => {
    test('should maintain mobile navigation functionality across orientations', async ({ page }) => {
      await page.goto('/business/dashboard');
      await mockAuthentication(page, 'employer');
      await page.reload();
      await waitForPageStable(page);
      ...
      if (await hamburgerButton.isVisible()) {
        ...
        const drawer = page.locator('[data-testid="mobile-drawer"], .mobile-drawer, [role="dialog"]').first();
        ...
        // Close drawer
        const closeButton = page.locator('[data-testid="close-drawer"], button[aria-label*="close"]').first();
        ...
        await waitForPageStable(page);
        await expect(page).toHaveScreenshot('navigation-landscape-after-close.png');
      }
    });

    test('should adapt navigation drawer size for landscape', async ({ page }) => {
      await page.goto('/business/dashboard');
      await mockAuthentication(page, 'employer');
      await page.reload();
      await waitForPageStable(page);
      ...
      if (await hamburgerButton.isVisible()) {
        ...
        if (await drawer.isVisible()) {
          ...
          await expect(page).toHaveScreenshot('navigation-drawer-landscape-adapted.png');
        }
      }
    });
  });
  ...
  test.describe('Modal Behavior During Orientation Changes', () => {
    test('should adapt modal sizing for orientation changes', async ({ page }) => {
      await page.goto('/business/applicants');
      await mockAuthentication(page, 'employer');
      await page.reload();
      await waitForPageStable(page);
  ...
  test.describe('Touch Target Consistency', () => {
    test('should maintain touch-friendly targets across orientations', async ({ page }) => {
      await page.goto('/login');
      await waitForPageStable(page);
      ...
      const testTouchTargets = async (orientation: string) => {
        ...
        for (let i = 0; i < Math.min(buttonCount, 5); i++) {
          ...
          if (await button.isVisible()) {
            ...
            if (boundingBox) {
              ...
              expect(boundingBox.height, `Button ${i} height in ${orientation}`).toBeGreaterThanOrEqual(40);
              expect(boundingBox.width, `Button ${i} width in ${orientation}`).toBeGreaterThanOrEqual(40);
            }
          }
        }
        ...
        for (let i = 0; i < Math.min(inputCount, 3); i++) {
          ...
          if (await input.isVisible()) {
            const boundingBox = await input.boundingBox();
            if (boundingBox) {
              expect(boundingBox.height, `Input ${i} height in ${orientation}`).toBeGreaterThanOrEqual(40);
            }
          }
        }
      };
