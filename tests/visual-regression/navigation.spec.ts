...
test.describe("Navigation Components - Visual Regression", () => {
  ...
  test("Hamburger menu button states - mobile", async ({ page }) => {
    ...
    await page.screenshot({
      path: "test-results/hamburger-closed-mobile.png",
      fullPage: true,
    });
    ...
    if (await hamburgerButton.isVisible()) {
      ...
      await page.screenshot({
        path: "test-results/hamburger-open-mobile.png",
        fullPage: true,
      });
    }
  });
  ...
  Object.values(["employer", "talent", "mentor"] as const).forEach(
    (userType) => {
      ...
      VIEWPORTS.mobile,
      {
        ...
        interactions: async (page) => {
          ...
          if (await hamburgerButton.isVisible()) {
            ...
            const profileSwitcher = page
              .locator('[data-testid="profile-switcher"], .profile-switcher')
              .first();
