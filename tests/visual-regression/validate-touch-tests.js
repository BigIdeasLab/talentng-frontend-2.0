...
try {
  ...
  console.log(
    `🧪 Test categories found: ${foundTests.length}/${requiredTests.length}`,
  );
  ...
  console.log(
    `🎯 Requirements coverage: ${foundRequirements.length}/${requirements.length}`,
  );
  ...
  if (
    foundTests.length === requiredTests.length &&
    foundRequirements.length >= requirements.length * 0.8
  ) {
    console.log(
