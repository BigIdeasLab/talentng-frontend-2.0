    ...
    test(`Opportunities list - ${viewport.name}`, async ({ page }) => {
      await testPageResponsive(
        ...
        viewport,
        {
          mockAuth: true,
          userType: 'employer',
          waitForSelector: '[data-testid="opportunities-table"], .opportunities-table, .responsive-table',
          fullPage: true,
        }
      );
    });
    ...
    test(`Opportunity details - ${viewport.name}`, async ({ page }) => {
      await testPageResponsive(
        ...
        viewport,
        {
          mockAuth: true,
          userType: 'employer',
          waitForSelector: '[data-testid="opportunity-details"], .opportunity-details, .opportunity-header',
          fullPage: true,
        }
      );
    });
