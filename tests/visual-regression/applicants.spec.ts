    ...
    test(`Applicant profile view - ${viewport.name}`, async ({ page }) => {
      await testPageResponsive(
        ...
        viewport,
        {
          mockAuth: true,
          userType: 'employer',
          waitForSelector: '[data-testid="applicant-profile"], .applicant-profile, .profile-header',
          fullPage: true,
        }
      );
    });
    ...
    test(`Hired talent list - ${viewport.name}`, async ({ page }) => {
      await testPageResponsive(
        ...
        viewport,
        {
          mockAuth: true,
          userType: 'employer',
          waitForSelector: '[data-testid="hired-talent-table"], .hired-talent-table, .responsive-table',
          fullPage: true,
        }
      );
    });
    ...
    await testPageResponsive(
      ...
      VIEWPORTS.mobile,
      {
        ...
        interactions: async (page) => {
          ...
          const actionsButton = page.locator('[data-testid="actions-dropdown"], button:has-text("Actions"), .actions-menu-trigger').first();
  ...
  test('Hire application modal - mobile', async ({ page }) => {
    await testPageResponsive(
      ...
      VIEWPORTS.mobile,
      {
        ...
        interactions: async (page) => {
          // Open hire modal
          const hireButton = page.locator('[data-testid="hire-applicant"], button:has-text("Hire")').first();
  ...
  test('Applicants with search - mobile', async ({ page }) => {
    await testPageResponsive(
      ...
      VIEWPORTS.mobile,
      {
        mockAuth: true,
        userType: 'employer',
        waitForSelector: '[data-testid="search-input"], input[placeholder*="Search"]',
        interactions: async (page) => {
