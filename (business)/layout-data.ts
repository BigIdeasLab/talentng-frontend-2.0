/**
 * Server-side data fetching for business layout
 * Fetches profile data once at layout level to avoid redundant requests
 * Data is passed to ProfileProvider and available to all business pages
 */
...
export async function getBusinessLayoutData() {
  try {
    ...
    console.error("Error loading business layout data on server:", error);
    return {
      profileData: null,
      profileRaw: null,
      userId: null,
