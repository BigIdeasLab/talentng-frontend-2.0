...
function runNetworkDiagnostics() {
  ...
  for (const [name, addresses] of Object.entries(interfaces)) {
    ...
    for (const addr of addresses) {
      if (addr.family === 'IPv4') {
        ...
        const type = addr.address.startsWith('192.168.') ? '📱 Mobile-friendly' : 
                    addr.address.startsWith('10.') ? '🏢 Corporate' :
                    addr.address.startsWith('172.') ? '🐳 Docker/VM' :
                    addr.address.startsWith('127.') ? '💻 Localhost' : '❓ Other';
  ...
  for (const addresses of Object.values(interfaces)) {
    ...
    for (const addr of addresses) {
      if (addr.family === 'IPv4' && !addr.internal) {
        ...
        } else if (addr.address.startsWith('172.') || addr.address.startsWith('10.')) {
  ...
  console.log('   netsh advfirewall firewall add rule name="Node.js Dev" dir=in action=allow protocol=TCP localport=8080');
