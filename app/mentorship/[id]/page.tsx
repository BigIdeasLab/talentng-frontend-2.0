...
import { getMentorById, getMentors } from "@/lib/api";
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const [mentorData, allMentors] = await Promise.all([
          getMentorById(id),
          getMentors(),
        ]);
        setMentor(mentorData);

        // Get similar mentors (exclude current one, limit to 3)
        const similar = allMentors.filter((m) => m.id !== id).slice(0, 3);
  ...
  if (!mentor) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-gray-500">Mentor not found</div>
      </div>
    );
  }
          ...
          <div className="space-y-6">
            <div className="space-y-2.5">
              ...
              <div className="flex items-center justify-between">
                ...
                <button
                  onClick={() => router.push("/talent/mentorship")}
