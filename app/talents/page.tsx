...
// Public talents interface (uses string IDs from API)
interface PublicTalentData {
  id: string; // String ID from public API
  userId: string;
  fullName: string;
  headline: string;
  location: string;
...
export default function TalentsPage() {
  ...
  const [talents, setTalents] = useState<PublicTalentData[]>([]);
  ...
  useEffect(() => {
    async function fetchTalents() {
      try {
        setLoading(true);
        setError(null);
        const data = await browseTalents({
        ...
        // Transform API response to match PublicTalentData interface
        const transformedData: PublicTalentData[] = data.map((talent) => {
          ...
          return {
            id: talent.id, // Use profile ID for linking (string)
            userId: talent.userId,
            fullName: talent.fullName,
            headline: talent.headline,
            location: talent.location,
            timesHired: talent.hiredCount || 0, // Use hiredCount from API
            ...
            stack: extractStrings(talent.stack), // This handles both string arrays and object arrays
            availability: Array.isArray(talent.availability)
              ? talent.availability
              : [],
            ...
            <>
              {talents.length === 0 ? (
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
                  {talents.map((talent) => (
                    <TalentCard key={talent.userId} talent={talent} />
                  ))}
                </div>
              )}
            </>
