...
export interface RateLimitInfo {
  ...
  type: 'throttler' | 'account_lockout' | 'unknown';
  message: string;
...
export function parseRateLimitError(error: any): RateLimitInfo {
  ...
  const isStrictAuthEndpoint = url.includes('/forgot-password') || url.includes('/reset-password');
  ...
  if (message.includes('ThrottlerException') || message.includes('Too Many Requests')) {
    if (isStrictAuthEndpoint) {
      ...
      userMessage = 'Too many password reset attempts. Please wait a minute before trying again.';
    } else if (isAuthEndpoint || isAdminEndpoint) {
      type = 'throttler';
      waitTime = 60; // Standard auth/admin endpoints: 5 requests per 60 seconds
      userMessage = 'Too many authentication attempts. Please wait a minute before trying again.';
    } else {
      type = 'throttler';
  ...
  if (message.includes('locked') || message.includes('lockout') || message.includes('temporarily locked')) {
    type = 'account_lockout';
    waitTime = 15 * 60; // Account lockout is 15 minutes
    userMessage = 'Account temporarily locked due to multiple failed login attempts. Please try again in 15 minutes.';
  }
  ...
  const retryAfterDate = waitTime ? new Date(Date.now() + waitTime * 1000) : undefined;
