...
export function getAccessibleColorPair(
  ...
  if (meetsWCAGStandard(contrastRatio, level, textSize)) {
    return { text: preferredText, background: preferredBackground, contrastRatio };
  }
...
export const ACCESSIBLE_TAILWIND_CLASSES = {
  ...
  interactive: {
    ...
    buttonSecondary: 'bg-gray-100 text-gray-900 hover:bg-gray-200 focus:ring-2 focus:ring-gray-500',
    link: 'text-blue-700 hover:text-blue-800 underline focus:ring-2 focus:ring-blue-500',
    input: 'border-gray-300 text-gray-900 focus:border-blue-500 focus:ring-2 focus:ring-blue-500',
  },
...
export function validateMobileColorContrast(
  ...
  if (!isValid) {
    if (context === 'touch-target') {
      recommendation = 'Use high contrast colors for touch targets on mobile devices';
