...
export function mobileOptimizedMemo<T extends React.ComponentType<any>>(
  ...
  const MemoizedComponent = memo(Component, propsAreEqual as any) as unknown as T;
...
export function useMobileOptimizedMemo<T>(
  ...
  const effectiveDeps = isMobile && simplifyOnMobile && mobileDeps ? mobileDeps : deps;
 ...
 */
export function withMobilePerformanceOptimization<T extends Record<string, any>>(
  ...
  if (enableProfiling && typeof window !== 'undefined' && process.env.NODE_ENV === 'development') {
...
export function useMobileOptimizedDebounce<T extends (...args: any[]) => any>(
  ...
  const [debouncedCallback, setDebouncedCallback] = React.useState<T | null>(null);
...
export function useMobileOptimizedThrottle<T extends (...args: any[]) => any>(
  ...
  }, [callback, effectiveLimit, inThrottle]) as T;
...
export const MobileLazyRender: React.FC<MobileLazyRenderProps> = ({
  ...
  const isMobile = useIsMobile();
  const [shouldRender, setShouldRender] = React.useState(!mobileOnly || !isMobile);
